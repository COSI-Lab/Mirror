		...
		for result.Next() {
			if result.Err() == nil {
				...
				if !ok {
					...
					mirrorErrors.Error("[0m InitNGINXStats can not parse distro to string", "warn")
					continue
				}
				...
				if !ok {
					...
					mirrorErrors.Error("[0m InitNGINXStats can not parse "+distro+" bytes to int"+distro+result.Record().String(), "warn")
					continue
				}
				...
				mirrorErrors.Error("[0m InitNGINXStats Flux Query Error"+result.Err().Error(), "warn")
			}
		}
	}

	log.Println("[INFO] InitNGINXStats successfully loaded previous stats from influxdb")
	mirrorErrors.Error("[0m InitNGINXStats successfully loaded previous stats from influxdb", "info")
}
