...
func main() {
	...
	*/
	...
	switch errorFrom {
	case "rsync":
		sendHook(fmt.Sprintf("%s: %s: %s", distro, rysncErrorCodes[code], time), url)
	case "nginxLogBreak":
		sendHook(fmt.Sprintf("%s: %s: %s", distro, "nginx log break", time), url)
	case "generic":
		sendHook(fmt.Sprintf("%s: %s: %s", distro, "Generic error", time), url)
	}
